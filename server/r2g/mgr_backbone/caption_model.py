...
import r2g.mgr_backbone.utils as utils
...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
            ...
            if divm > 0:
                ...
                for prev_choice in range(divm):
                    ...
                    for prev_labels in range(bdash):
                        change.scatter_add_(
                            1,
                            prev_decisions[:, prev_labels].unsqueeze(-1),
        ...
        def beam_step(
            ...
            candidate_logprobs = (
                beam_logprobs_sum.unsqueeze(-1) + logprobs
            )  # beam_logprobs_sum Nxb logprobs is NxbxV
            ys, ix = torch.sort(
                candidate_logprobs.reshape(candidate_logprobs.shape[0], -1), -1, True
            )
            ...
            if t > 0:
                ...
                assert (
                    ...
                    == beam_seq.reshape(-1, beam_seq.shape[-1])[state_ix].view_as(
                        beam_seq
                    )
                ).all()
            ...
            beam_seq = torch.cat(
                [beam_seq, selected_ix.unsqueeze(-1)], -1
            )  # beam_seq Nxbxl
            beam_logprobs_sum = beam_logprobs_sum.gather(1, beam_ix) + logprobs.reshape(
                batch_size, -1
            ).gather(1, ix)
        ...
        # INITIALIZATIONS
        beam_seq_table = [
            torch.LongTensor(batch_size, bdash, 0).to(device) for _ in range(group_size)
        ]
        beam_seq_logprobs_table = [
            torch.FloatTensor(batch_size, bdash, 0, self.vocab_size + 1).to(device)
            for _ in range(group_size)
        ]
        beam_logprobs_sum_table = [
            torch.zeros(batch_size, bdash).to(device) for _ in range(group_size)
        ]
        ...
        args = utils.split_tensors(
            group_size, args
        )  # For each arg, turn (Bbg)x... to (Bb)x(g)x...
        ...
        for t in range(self.max_seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    if (
                        suppress_UNK
                        and hasattr(self, "vocab")
                        and self.vocab[str(logprobs.size(1) - 1)] == "UNK"
                    ):
                        logprobs[:, logprobs.size(1) - 1] = (
                            logprobs[:, logprobs.size(1) - 1] - 1000
                        )
                        ...
                        for vix in range(bdash):
                            if is_end[vix]:
                                final_beam = {
                                    ...
                                    "logps": beam_seq_logprobs_table[divm][
                                        b, vix
                                    ].clone(),
                                    "unaug_p": beam_seq_logprobs_table[divm][b, vix]
                                    .sum()
                                    .item(),
    ...
    def old_beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        # INITIALIZATIONS
        beam_seq_table = [
            torch.LongTensor(self.max_seq_length, bdash).zero_()
            for _ in range(group_size)
        ]
        beam_seq_logprobs_table = [
            torch.FloatTensor(self.max_seq_length, bdash, self.vocab_size + 1).zero_()
            for _ in range(group_size)
        ]
        ...
        for t in range(self.max_seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    for vix in range(bdash):
                        if (
                            beam_seq_table[divm][t - divm, vix] == self.eos_idx
                            or t == self.max_seq_length + divm - 1
                        ):
                            final_beam = {
